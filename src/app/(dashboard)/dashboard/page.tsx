...
export default function DashboardPage() {
  ...
  async function loadDashboardData(user: User): Promise<void> {
    setLoading(true);
    try {
      // Attempt to call demoApi methods if available; fall back to safe defaults.
      const [
        activityData,
        jobsData,
        contractsData
      ] = await Promise.all([
        // demoApi method names may vary; these are the likely ones.
        Promise.resolve([]),
        (user.role === 'client' ? demoApi.getJobsByClientId(user.id) : Promise.resolve([])),
      ...
      if (statsData) {
        setStats({
          ...
          hiredFreelancers: statsData.hiredFreelancers ?? 0
        });
      } else {
        // ensure defaults if API didn't return stats
        setStats(prev => ({ ...prev }));
      }
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 mb-2">
                  {isFreelancer ? 'Pro Tip' : 'Success Tip'}
                </h3>
